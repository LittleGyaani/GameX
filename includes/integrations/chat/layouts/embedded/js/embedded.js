    ...
    $.ccembedded = (function(){
        ...
        return {
                ...
                $('.cometchat_module').click(function(){
                    ...
                    if(id[2] == 'block'){
                        ...
                        if (jqcc.cometchat.membershipAccess('bots','extensions')){
                            ...
                            var content = '<iframe id="cometchat_bots_iframe" src="'+baseUrl+'extensions/bots/index.php?cc_layout=embedded" style="height:90%;border:none;"></iframe>';
            ...
            buddyList: function(item){
                ...
                $("span.cometchat_userscontentavatar").find("img").attr('onError',"this.onerror=null;this.src='"+staticCDNUrl+"admin/images/pixel.png'");
            ...
            userStatus: function(item){
                ...
                jqcc.cometchat.setThemeVariable('customStatusMessage', item.m);
                if(item.s != 'away'){
                    jqcc.cometchat.setThemeVariable('currentStatus', item.s);
                }
            ...
            createChatboxData: function(id, name, status, message, avatar, link, isdevice, silent, tryOldMessages, restored){
                ...
                $("div.cometchat_userscontentavatar").find("img").attr('onError',"this.onerror=null;this.src='"+staticCDNUrl+"admin/images/pixel.png'");
            },
            ...
            addMessages: function(item,silent){
                ...
                $.each(item, function(i, incoming){
                    ...
                    var alreadyRecieved = recievedMessages.indexOf(parseInt(incoming.id));
                    if(incoming.id != 'undefined' && alreadyRecieved == -1){
                        recievedMessages.push(parseInt(incoming.id));
                    }
                    ...
                    if(jqcc.cometchat.getThemeArray('trying', incoming.from)==undefined){
                        if(typeof (jqcc[settings.theme].createChatbox)!=='undefined' && incoming.nopopup == 0 && silent != 1 && jqcc.cometchat.getCcvariable()['timestamp']<incoming.id){
                    ...
                    if(settings.autoPopupChatbox==1 && incoming.self==0 && incoming.old!=1 && $('#cometchat_user_'+incoming.from+'_popup .cometchat_prependMessages').length==0 && jqcc.cometchat.getCcvariable()['timestamp']<incoming.id) {
                    ...
                    if(incoming.self!=1&&incoming.old!=1 && ((typeof(alreadyreceivedunreadmessages[incoming.from])!='undefined'&& alreadyreceivedunreadmessages[incoming.from]<incoming.id) || typeof(alreadyreceivedunreadmessages[incoming.from])=='undefined')){
                        if (incoming.self != 1 && settings.messageBeep == 1 && alreadyRecieved == -1) {
                            if ((typeof $.cookie(settings.cookiePrefix+"sound") == 'undefined' || $.cookie(settings.cookiePrefix+"sound") == null) || $.cookie(settings.cookiePrefix + "sound") == 'true') {
                    ...
                    if(incoming.self !=1 && incoming.old!=1 && $('#cometchat_user_'+incoming.from+'_popup').length==0 )
                    {
                        ...
                        if(incoming.old!=1 && incoming.self!=1 && alreadyRecieved == -1){
                            if((typeof $.cookie(settings.cookiePrefix+"sound") == 'undefined' || $.cookie(settings.cookiePrefix+"sound") == null) || $.cookie(settings.cookiePrefix+"sound")=='true'){
                ...
                $("span.cometchat_other_avatar").find("img").attr('onError',"this.onerror=null;this.src='"+staticCDNUrl+"admin/images/pixel.png'");
            },
            ...
            updateStatus: function(status){
                language = jqcc.cometchat.getLanguage();
                customStatus = jqcc.cometchat.getThemeVariable('customStatusMessage');
                ...
                var statusMessages = [language[30], language[31], language[32], language[34]];

                if(statusMessages.indexOf(customStatus) == -1){
                ...
                $("#cometchat_self .cometchat_userdisplaystatus").text(statusMessage);
            },
