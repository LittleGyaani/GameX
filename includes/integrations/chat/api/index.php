...
switch ($_REQUEST['action']) {
	...
	case 'sendmessage':
		...
		if(!empty($_REQUEST['senderID'])){
			$userid = $_REQUEST['senderID'];
		} else {
			$isSendMessageFlag = false;
			$response = array('failed' => array('status' => '1010', 'message' => "Sender ID can not be Empty."));
		}

		if(!empty($_REQUEST['receiverID'])){
			$to = $_REQUEST['receiverID'];
		} else {
			$isSendMessageFlag = false;
			$response = array('failed' => array('status' => '1010', 'message' => "Receiver ID can not be Empty."));
		}
		...
		if($isSendMessageFlag == true) {
			if(empty($_REQUEST['isGroup'])) {
				...
				if(empty($message)) {
					$response = array('failed' => array('status' => '1010', 'message' => "You can not post empty Message."));
				} else {
					$type = '';
					sendMessage($to,$message,$dir,$type);
					$response = array('success' => array('status' => '1000', 'message' => "Message Send to One-One Successfully.", 'userid' => $userid));
				}
	
			} else {
	
				$messagetype = ''; $notsilent = 0; $styleStart = ''; $styleEnd = ''; global $cookiePrefix;
				...
				if(empty($message)) {
					...
					if($isGroupJoined) {
						$query = sql_query('insertGroupMessage',array('userid'=>$userid, 'to'=>$to, 'styleStart'=>$styleStart, 'message'=>$message, 'styleEnd'=>$styleEnd, 'timestamp'=> getTimeStamp()));
						$insertedid = sql_insert_id('cometchat_chatroommessages');
						$response = array('success' => array('status' => '1000', 'message' => "Message Send to Group Successfully.", 'userid' => $userid));
					} else {
						$response = array('failed' => array('status' => '1010', 'message' => "You dont have Permission to Send  Message to this Group."));
					}
				}
			}
		}
