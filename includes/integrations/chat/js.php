...
if(!empty($_REQUEST['admin'])){
	...
	$adminjs = $cacheDir.DIRECTORY_SEPARATOR.'admin'.$adminappjs.'.js';
	...
	$jsfile = $cacheDir.DIRECTORY_SEPARATOR.$cbfn.$type.$name.$layout.$color.$lang.$enablecustomjs.$forcedockedenable.'.js';
	$cometchat = array();
	if(file_exists($jsfile)&&DEV_MODE!=1){
		...
		if(($type!='core'||$name!='default')&&($type!='extension'||($type=='extension'&&$name=='jabber'))&&($type!='external')){
			...
			$cometchat['settings']['client'] = $GLOBALS['client'];
			...
			$cometchat['settings']['crguestsMode'] = $crguestsMode;

		   	if(function_exists('hooks_customSettings')){
				hooks_customSettings();
			}
