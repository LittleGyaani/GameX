...
/**
* $writableDir - Cache Folder Path
* @var string
*/
$writableDir = dirname(__FILE__).DIRECTORY_SEPARATOR.'writable';

/**
* $cacheDir - Cache Folder Path
* @var string
*/
$cacheDir = $writableDir.DIRECTORY_SEPARATOR.'cache'.(!empty($client)?DIRECTORY_SEPARATOR.$client:'');

/**
* check if $cache directory is present and assign folder permission as 777 to Cache Folder.
* @var string
*/
if (!is_dir($cacheDir)) {
...
if(!empty($api_response)){
	$preEnabledPlugins = array('audiochat','avchat','block','voicenote','filetransfer','whiteboard','writeboard','smilies','stickers');

	foreach ($preEnabledPlugins as $value) {
		if(!in_array($value, $api_response['plan']['plugins'])){
			unset($preEnabledPlugins[$value]);
		}
	}
	$plugins = $crplugins = $preEnabledPlugins;
}else{
	$plugins = $crplugins = array('smilies','stickers');
}
