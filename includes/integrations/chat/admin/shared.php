...
function clearcachejscss($directory, $recursive = true, $listDirs = false, $listFiles = true, $exclude = '') {
    global $cacheDir;
    deleteFolderContent($cacheDir,array("index.html"),0);
}
...
function updateAPIResponse($latest_response){
    ...
    */
    ...
    if(!empty($latest_response['plan']['features'])){
        if(in_array('rbac', $latest_response['plan']['features'])){
            $newSettings['rbac'] = 1;
        }
        if(in_array('credits', $latest_response['plan']['features'])){
            $newSettings['credits'] = 1;
        }
        if(in_array('analytics', $latest_response['plan']['features'])){
            $newSettings['analytics'] = 1;
        }
    }
    if(!empty($client)){
