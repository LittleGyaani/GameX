...
function fetchChatroomMessages($params){
	global $response, $userid, $lastMessages, $guestsMode, $crguestsMode, $lastgroupmessageid, $chatrooms_language, $cookiePrefix, $guestnamePrefix, $trayicon, $groupid, $callbackfn, $request_rttlang, $client;
	...
	if(count($joinedgroups) > 0){
		...
		if(!empty($groupid) && $force == 1 && !empty($_SESSION['cometchat']['cometchat_chatroom_'.$groupid])){
			...
			$key = 'groupMessages';
			if(!empty($client)){
				$key = 'groupMessages_cloud';
			}
			$query = sql_query($key,array('timestampCondition'=>$timestampCondition, 'guestpart'=>$guestpart, 'limitClause'=>$limitClause));
...
function invite(){
	...
	$s['busy'] = '';
	$s['offline'] = '';
	$staticCDNURLDefaultAvatar = STATIC_CDN_URL.'admin/images/pixel.png';
	foreach ($buddyList as $buddy){
		...
		if($buddy['id'] != $userid){
			...
			<img class="useravatar" height=30 width=30 src="'.$buddy['a'].'" onerror="this.onerror=null;this.src=\''.$staticCDNURLDefaultAvatar.'\';"/>
			</div>
...
function deleteChatroomMessage(){
	...
	if(!empty($deleteflag)){
		...
		$affectedrow = sql_affected_rows($query);
		if ($affectedrow == 0) {
			$query = sql_query('deleteGroupMessageFromArchive',array('id'=>$groupmessageidtodelete));
			$affectedrow = sql_affected_rows($query);
		}
		if($affectedrow > 0){
			$response['success'] = true;
