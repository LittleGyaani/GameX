...
$sql_queries['admin_users_count'] = "select count(userid) as total from cometchat_users";
$sql_queries['admin_getUsers'] = "select * from cometchat_users order by userid desc limit {start},{limit}";
...
$sql_queries['admin_users'] = "select username, uid, userid from cometchat_users where username LIKE '%{username}%' limit 10";

$sql_queries['get_user_deatils'] = "select userid, username from cometchat_users where uid ='{uid}'";
$sql_queries['get_user_deatils_with_id'] = "select * from cometchat_users where userid ={userid}";
...
$sql_queries['getChatroomData_cloud'] = "select DISTINCT cometchat_chatroommessages.id id, cometchat_chatroommessages.message, cometchat_chatroommessages.sent, coalesce(m.displayname, m.{usertable_username}) `from`, cometchat_chatroommessages.userid fromid, m.{usertable_userid} userid from {tbl} as cometchat_chatroommessages join {usertable} m on m.{usertable_userid} = cometchat_chatroommessages.userid  where cometchat_chatroommessages.chatroomid = '{chatroomid}' and cometchat_chatroommessages.message not like '%banned_%' and cometchat_chatroommessages.message not like '%kicked_%' and cometchat_chatroommessages.message not like '%deletemessage_%' {prependCondition} {guestpart} order by id desc limit {limit}";
$sql_queries['getChatroomDetails'] = "select * from cometchat_chatrooms where cometchat_chatrooms.id = '{id}'";
$sql_queries['cloudapi_getChatroomsList'] = "select id,name,lastactivity from cometchat_chatrooms";
$sql_queries['cloudapi_deleteGroups'] = "delete from cometchat_chatrooms,cometchat_chatrooms_users using cometchat_chatrooms inner join cometchat_chatrooms_users on cometchat_chatrooms.id = cometchat_chatrooms_users.chatroomid where cometchat_chatrooms.id in ({GroupsList})";
...
$sql_queries['getFriends'] = "select userid, friends from `cometchat_users` where uid = '{uid}'";
$sql_queries['checkUserExists'] = "select userid from `cometchat_users` where uid = '{uid}'";
...
$sql_queries['getGroupId'] = "select id, type from cometchat_chatrooms where guid = '{guid}' or id = '{guid}'";
$sql_queries['getGroupByGUID'] = "select id, type, name, createdby from cometchat_chatrooms where guid = '{guid}'";
...
$sql_queries['cloudapi_addGroupUser'] = "insert into cometchat_chatrooms_users (chatroomid,userid) values ('{GUID}', '{userid}') ";
$sql_queries['cloudapi_removeGroupUser'] = "DELETE FROM `cometchat_chatrooms_users` WHERE userid in(select userid from cometchat_users where userid in ('{userid}') and chatroomid='{roomId}') ";
...
$sql_queries['isUserBlocked'] = "select * from cometchat_block where fromid = '{fromuserid}' and toid = '{touserid}'";
$sql_queries['getUserByUID'] = "select * from cometchat_users where uid = '{fromuserid}' or uid = '{touserid}'";
...
$sql_queries['deleteGroupByGUID'] = "delete from cometchat_chatrooms where guid = '{guid}' and createdby != 0 ";
...
$sql_queries['deleteGroupMessageFromArchive'] = "delete from cometchat_chatroommessages_archive where id='{id}'";
...
$sql_queries['cloud_getGroupId'] = "select id, type from cometchat_chatrooms where guid = '{GUID}'";
$sql_queries['cloud_checkUserExists'] = "select cometchat_users.userid as userid from cometchat_users where uid = '{value}'";
...
$sql_queries['oneononeEmailsentTillMsgID'] = "select value from cometchat_settings where setting_key='oneononeEmailsentTillMsgID'";
$sql_queries['lastGroupMessageID'] = "select value from cometchat_settings where setting_key='lastGroupMessageID'";
...
class SqlQueries{
	function getQueries(){
		...
		$sqlqueries['cloudapi_getUserDetails'] = "select `userid`,`username`,`link`,`avatar`,`displayname`,friends from `".TABLE_PREFIX.DB_USERTABLE."` {where}";
		$sqlqueries['cloudapi_getUser'] = "select uid,`username` as name ,`link`,`avatar`,`friends`,`role`,`grp` as groups,`credits`,`userid` as cid from `".TABLE_PREFIX.DB_USERTABLE."` {where}";
		$sqlqueries['getUserJoinedGroupDetails']=" select * from `cometchat_chatrooms_users` left join cometchat_chatrooms on cometchat_chatrooms_users.chatroomid=cometchat_chatrooms.id where cometchat_chatrooms_users.userid={id} ";
		...
		$sqlqueries['groupMessages_cloud'] = "select DISTINCT cometchat_chatroommessages.id id, cometchat_chatroommessages.message, cometchat_chatroommessages.chatroomid, cometchat_chatroommessages.sent, coalesce(m.displayname, m.".DB_USERTABLE_NAME.") `from`, cometchat_chatroommessages.userid fromid, m.".DB_USERTABLE_USERID." userid from cometchat_chatroommessages join ".TABLE_PREFIX.DB_USERTABLE." m on cometchat_chatroommessages.userid = m.".DB_USERTABLE_USERID." where {timestampCondition} cometchat_chatroommessages.message not like 'banned_%' and cometchat_chatroommessages.message not like 'kicked_%' and cometchat_chatroommessages.message not like 'deletemessage_%' {guestpart} order by id desc {limitClause}";
    ...
    $sqlqueries['cloudapi_removeUserUsingUID'] = "delete from ".TABLE_PREFIX.DB_USERTABLE." where uid = '{uid}'";
    $sqlqueries['removeIdsFromOtherFriendListUsingUID'] = "SELECT *  FROM  ".TABLE_PREFIX.DB_USERTABLE." WHERE CONCAT(',',friends,',') LIKE '%,{uid},%'";
    $sqlqueries['cloudapi_updateuserUsingUID'] = "update ".TABLE_PREFIX.DB_USERTABLE." set {set} where `uid` = '{uid}'";
    $sqlqueries['cloudapi_updateFriendsUsingUID'] = "update ".TABLE_PREFIX.DB_USERTABLE." set `friends` = '{friends}' where uid = '{uid}'";
    $sqlqueries['cloudapi_getUsersDetails'] = "SELECT userid, username, displayname, friends, uid, role FROM ".TABLE_PREFIX.DB_USERTABLE." LIMIT {limit} OFFSET {offset}";

    $sqlqueries['cloudapi_getCredits'] = "SELECT credits FROM ".TABLE_PREFIX.DB_USERTABLE." WHERE uid = '{uid}'";
...
$sqlqueries['all_active_users'] = "SELECT COUNT(userid) AS usercount, IF(userid > {firstguestid}, 'guest', 'user') AS usertype, SUBSTR(MONTHNAME, 1, 3) AS month, MONTH AS MONTHNO, SUBSTR(YEAR, 3, 2) AS year FROM ( SELECT * FROM ( SELECT userid AS userid, MONTHNAME, MONTH, YEAR FROM ( SELECT cometchat_chatroommessages.userid AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat_chatroommessages GROUP BY userid, MONTHNAME UNION SELECT cometchat_chatroommessages_archive.userid AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat_chatroommessages_archive GROUP BY userid, MONTHNAME ) AS temp ) AS groups UNION SELECT * FROM ( SELECT userid, MONTHNAME AS MONTHNAME, MONTH, YEAR FROM ( SELECT cometchat.from AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat GROUP BY userid, MONTHNAME UNION SELECT cometchat_archive.from AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat_archive GROUP BY userid, MONTHNAME ) AS temp ) AS oneoneone ) AS activeusers GROUP BY MONTHNAME, YEAR, usertype ORDER BY year,MONTHNO";
	...
	$sqlqueries['fetchUnreadMessagesforOfflineUsers'] = "select *, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." AS toname FROM ( SELECT MAX(cometchat.id) AS messageid, cometchat.from, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." AS fromname, cometchat.to, cometchat_status.custom_data as mail_sent, COUNT(cometchat.id) as unreadmessage, GROUP_CONCAT(cometchat.message separator '~') as messages, cometchat.sent, cometchat.read, cometchat.direction, cometchat_status.lastactivity FROM cometchat LEFT JOIN ".TABLE_PREFIX.DB_USERTABLE." ON cometchat.from = ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." LEFT JOIN cometchat_status ON cometchat.to = cometchat_status.userid WHERE cometchat.read <> 1 AND cometchat.direction < 2 AND cometchat.id > {oneononeEmailsentTillMsgID} AND( cometchat_status.status IS NULL OR cometchat_status.status = 'offline' OR cometchat_status.lastactivity < {time}) GROUP BY cometchat.from,cometchat.to ) AS messagetble LEFT JOIN ".TABLE_PREFIX.DB_USERTABLE." ON messagetble.to = ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID."  ORDER BY messagetble.messageid";

	$sqlqueries['fetchGroupMessagesforOfflineUsers'] = "select  *, GROUP_CONCAT(group_userid separator '~') as group_userid,GROUP_CONCAT(group_username separator '~') as group_username from ( select cometchat_chatroommessages.id as messageid, cometchat_chatroommessages.chatroomid, cometchat_chatrooms.name AS groupname, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." messagefrom, cometchat_chatroommessages.userid AS fromuserid, cometchat_chatroommessages.message, cometchat_chatroommessages.sent, cometchat_chatroommessages.custom_data FROM `cometchat_chatroommessages` LEFT JOIN ".TABLE_PREFIX.DB_USERTABLE." ON cometchat_chatroommessages.userid = ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." LEFT JOIN cometchat_chatrooms ON cometchat_chatroommessages.chatroomid = cometchat_chatrooms.id where cometchat_chatroommessages.id >{lastGroupMessageID} ) as groupmessages left join  ( select ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." as group_userid, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." group_username, cometchat_status.lastactivity lastactivity, cometchat_chatrooms_users.chatroomid as groupid FROM ".TABLE_PREFIX.DB_USERTABLE." LEFT JOIN cometchat_status ON ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid INNER JOIN cometchat_chatrooms_users ON ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_chatrooms_users.userid AND	cometchat_status.lastactivity < {time}) as groupusers ON groupmessages.chatroomid = groupusers.groupid GROUP BY messageid";

	$sqlqueries['update_email_count'] = "update `cometchat_status` set `custom_data`='{data}' where userid='{userid}'";
