...
function sql_getQuery($key, $params = array()){
	...
	$sql = $sql_queries[$key];
	$excludeEscapeforKeys = array('querystring', 'timestampCondition', 'condition', 'prepend', 'prependCondition', 'guestpart', 'originalsql', 'extra', 'sqlpart', 'insertvalues', 'limitClause', 'usertable', 'set', 'userids','avatarfield','avatartable','where');
	if(!empty($params)){
...
function cometchatMemcacheConnect(){
	...
	if(defined('MEMCACHE') && MEMCACHE!=0 && MC_NAME=='memcachier'){
		...
		phpFastCache::setup("path",$cacheDir);
...
function sanitize($text) {
	...
	$search = "/((?#Email)(?:\S+\@)?(?#Protocol)(?:(?:ht|f)tp(?:s?)\:\/\/|~\/|\/)?(?#Username:Password)(?:\w+:\w+@)?(?#Subdomains)(?:(?:[-\w]+\.)+(?#TopLevel Domains)(?:com|org|net|gov|mil|biz|info|mobi|name|aero|jobs|museum|travel|me|xyz|restaurent|rest|guru|club|a[cdefgilmnoqrstuwz]|b[abdefghijmnorstvwyz]|c[acdfghiklmnoruvxyz]|d[ejkmnoz]|e[ceghrst]|f[ijkmnor]|g[abdefghilmnpqrstuwy]|h[kmnrtu]|i[delmnoqrst]|j[emop]|k[eghimnprwyz]|l[abcikrstuvy]|m[acdeghklmnopqrstuvwxyz]|n[acefgilopruz]|om|p[aefghklmnrstwy]|qa|r[eouw]|s[abcdeghijklmnortuvyz]|t[cdfghjkmnoprtvwz]|u[augkmsyz]|v[aceginu]|w[fs]|y[etu]|z[amw]|aero|arpa|biz|com|coop|edu|info|int|gov|mil|museum|name|net|org|pro))(?#Port)(?::[\d]{1,5})?(?#Directories)(?:(?:(?:\/(?:[-\w~!$+|.,=]|%[a-f\d]{2}|[&])+)+|\/)+|#)?(?#Query)(?:(?:\?(?:[-\w~!$+|\/.,*:]|%[a-f\d{2}])+=?(?:[-\w~!$+|.,*:=]|%[a-f\d]{2})*)(?:&(?:[-\w~!$+|.,*:]|%[a-f\d{2}])+=?(?:[-\w~!$+|.,*:=]|%[a-f\d]{2})*)*)*(?#Anchor)(?:#(?:[-\w~!$+|\/.,*:=]|%[a-f\d]{2})*)?)([^[:alpha:]]|\?)/i";
...
function getChatroomData($chatroomid, $prelimit = 0, $lastMessages = 0) {
	global $guestsMode, $crguestsMode, $guestnamePrefix, $language, $userid, $cookiePrefix, $plugins, $trayicon, $messages, $client;
	...
	if(empty($prelimit) && empty($lastMessages)) {
		...
		*/
		$primarygroupmessges = sql_query($key,array('tbl'=>'cometchat_chatroommessages','chatroomid'=>$chatroomid, 'prependCondition'=>$prependCondition, 'guestpart'=>$guestpart, 'limit'=>$messagelimit, 'usertable'=>$usertable, 'usertable_username'=>$usertable_username, 'usertable_userid'=>$usertable_userid));
		$messagecount = sql_num_rows($primarygroupmessges);
		addMoreGroupMessages($primarygroupmessges, $chatroomid);

		if($messagecount < $messagelimit){
			$archivegroupmessges = sql_query($key,array('tbl'=>'cometchat_chatroommessages_archive','chatroomid'=>$chatroomid, 'prependCondition'=>$prependCondition, 'guestpart'=>$guestpart, 'limit'=>($messagelimit-$messagecount), 'usertable'=>$usertable, 'usertable_username'=>$usertable_username, 'usertable_userid'=>$usertable_userid));
...
function getStatus() {
	...
	if(sql_num_rows($query) > 0) {
		...
		$viewProfile = fetchLink($chat['link']);
		if($viewProfile == null || $viewProfile == "null"){
			$viewProfile = '';
		}
...
function getRole($userid) {
	global $integration,$response,$cacheDir,$client;
	$role = 'default';
	if(empty($_SESSION['cometchat'])){
		$_SESSION['cometchat'] = array();
	}
...
function sendNotificationMail($from,$to,$subject,$htmlTemplate){
	global $smtpHost, $smtpEmail, $smtpEmailPassword, $fromName, $fromEmail;
	...
	if (empty($fromEmail) && filter_var($fromEmail, FILTER_VALIDATE_EMAIL) ) {
		...
		$response['message'] 	= "Invalid or blank from email address";
	}

	if (empty($to) && !is_array ($to) && filter_var($to, FILTER_VALIDATE_EMAIL) ) {
		$response['status'] 	= false;
		$response['message'] 	= "Invalid or blank to email address";
	}

	if (empty($htmlTemplate)) {
		$response['status'] 	= false;
		$response['message'] 	= "Empty message body";
	}

	if ($response['status']) {
	    $filename = dirname(__FILE__).DIRECTORY_SEPARATOR."functions".DIRECTORY_SEPARATOR."phpmailer".DIRECTORY_SEPARATOR."PHPMailerAutoload.php";
	    ...
	    */
	    ...
	    $mail->Password = $smtpEmailPassword;
...
function processMessages($message){
	if (strpos($message, 'stickers') > 0) {
		$stickersAPIResponse = getCachedSettings('stickersAPIResponse');
		$stickersData = json_decode($stickersAPIResponse,true);
		$message = str_replace("CC^CONTROL_", "", $message);
		$message = json_decode($message,true);
		$stickerCategory = $message['params']['category'];
		$stickerName = strtolower($message['params']['key']);
		$stickerName = $stickerCategory."/".str_replace(" ", "-", $stickerName).'.png';

		$featuredUrl = $stickersData['stickers'][$stickerCategory]['featured'];
		$featuredUrl = substr($featuredUrl, 0, stripos($featuredUrl, $stickerCategory."/"));
		$featuredUrl = $featuredUrl.$stickerName;
		return '<img class="cometchat_stickerImage" type="image" src="'.$featuredUrl.'">';
	}

	if (strpos($message, 'cometchat_smiley') > 0) {
		return str_replace('class="cometchat_smiley"', 'class="cometchat_smiley" type="image"', $message);
	}

	if (strpos($message, 'mobileAction="javascript') > 0) {
		return '';
	}

	if (strpos($message, "CC^CONTROL_") > 0) {
		return '';
	}
	return $message;
}
