...
(function($){
	$.cometchat = $.cometchat||function(){
					...
					success: function(data){
						...
						if(data){
							...
							$.each(data, function(type, item){
								...
								if(typeof (jqcc[calleeAPI].customHeartbeatData)!=='undefined'){
									jqcc[calleeAPI].customHeartbeatData(type, item);
								}
								if(type=='buddylist'){
								...
								if(jqcc.cometchat.getCcvariable().loggedinusertype == 'guestuser'){
									jqcc[calleeAPI].showLoggedOutLayout(item);
								}
								if(type=='role'){
									role = item;
								}
								...
								if(type=='userstatus'){
									...
									if(typeof (jqcc[calleeAPI].userStatus)!=='undefined'){
										...
										if (settings.client != '' && settings.theme == "docked") {
											var controlparameters = {"cc_protocol":location.protocol, "cod":settings.client, "basedata":ccvariable.baseData};
											controlparameters = JSON.stringify(controlparameters);
											parent.postMessage('DM^CONTROL_'+controlparameters,'*');
										}
									}
								...
								if(type=='initialize'){
									...
									ccvariable.externalVars["lastrecievedid"] = item;
		...
		/**
		* launch
		* @params uid or {'guid':''} for groups
		* return open respective chatbox
		*/
		arguments.callee.launch = function(params){
			if(typeof(params) == 'undefined' || params == 'null' || params == 0 || params == ''){
				return;
			}
			// Launch Group Chat using GUID
			if(typeof(params) == 'object' && params.hasOwnProperty('guid')){
				jqcc.cometchat.launchGroupByGUID(params.guid);
			}

			// Launch Private Chat Using UID
			if(typeof(params) == 'object' && params.hasOwnProperty('uid')){
				jqcc.cometchat.chatWithUID(params.uid,1);
			}
		};

		/**
		* getGroupId
		* @params guid for group chat
		* return
		*/
		arguments.callee.launchGroupByGUID = function(guid){
			var baseURL = jqcc.cometchat.getBaseUrl();

			jqcc.ajax({
				url: baseURL+"cometchat_update.php",
				data: {guid: guid},
				dataType: 'jsonp',
				type: jqcc.cometchat.getCcvariable()['dataMethod'],
				timeout: jqcc.cometchat.getCcvariable()['dataTimeout'],
				success: function(data){
		...
		arguments.callee.getUnreadMessageCount = function(params){
			var cc_state = jqcc.cookie(settings.cookiePrefix+'state');
			if(cc_state==null){
				return 0;
			}
			...
			$.ajax({
				...
				success: function(data){
					...
					if(typeof jqcc[settings.theme].statusSendMessageSuccess != "undefined") {
						...
						jqcc.cometchat.setThemeVariable('customStatusMessage');
					}
				},
				error: function(data){
		...
		arguments.callee.audiovideocall = function(toid,cccloud){
			if(toid == 0 || toid == ''){
				return;
			}
			if(typeof(jqcc.cometchat.audiovideocallWithUID) == 'function' && typeof(cccloud) != 'undefined'){
			   jqcc.cometchat.audiovideocallWithUID(toid);
		    }else{
		...
		arguments.callee.audiocall = function(toid,cccloud){
			if(toid == 0 || toid == ''){
				return;
			}
			if(typeof(jqcc.cometchat.audiocallWithUID) == 'function' && typeof(cccloud) != 'undefined'){
			   jqcc.cometchat.audiocallWithUID(toid);
		    }else{
				var getActivePlugins = jqcc.cometchat.getSettings().plugins;
				if(!getActivePlugins.includes('audiochat')){
		...
		arguments.callee.screenshare = function(toid,cccloud){
			if(toid == 0 || toid == ''){
				return;
			}
			if(typeof(jqcc.cometchat.screenshareWithUID) == 'function' && typeof(cccloud) != 'undefined'){
			   jqcc.cometchat.screenshareWithUID(toid);
		    }else{
				var getActivePlugins = jqcc.cometchat.getSettings().plugins;
				if(!getActivePlugins.includes('screenshare')){
					var screenshareDisable = jqcc.ccscreenshare.getLanguage('screenshare_disabled');
					alert(screenshareDisable);
					return;
				}
				var caller = jqcc.cometchat.getUserID();
				jqcc['ccscreenshare'].init({'to':toid, 'caller': caller, 'chatroommode': 0});
			}
		},
		arguments.callee.whiteboard = function(toid,cccloud){
			if(toid == 0 || toid == ''){
				return;
			}
			if(typeof(jqcc.cometchat.whiteboardWithUID) == 'function' && typeof(cccloud) != 'undefined'){
			   jqcc.cometchat.whiteboardWithUID(toid);
		    }else{
				var getActivePlugins = jqcc.cometchat.getSettings().plugins;
				if(!getActivePlugins.includes('whiteboard')){
					var whiteboardDisable = jqcc.ccwhiteboard.getLanguage('whiteboard_disabled');
					alert(whiteboardDisable);
					return;
				}
				var caller = jqcc.cometchat.getUserID();
				jqcc['ccwhiteboard'].init({'to':toid, 'caller': caller, 'chatroommode': 0});
			}
		}
		arguments.callee.addUsertoGroup = function(groupid,userid,cccloud){
			var groupType = 3;
            var groupsDetails = jqcc.cometchat.getChatroomVars('chatroomdetails');

            for(var group in groupsDetails){
                if(groupsDetails[group].hasOwnProperty('id') && groupid == groupsDetails[group].id && groupsDetails[group].hasOwnProperty('type')){
                    groupType = groupsDetails[group].type;
                }
            }

			if(typeof(jqcc.cometchat.getUserFromUID) == 'function' && typeof(cccloud) != 'undefined'){
	            jqcc.ajax({
	                url: baseUrl+"cometchat_update.php",
	                data: {uid: userid},
	                cache: false,
	                ...
	                success: function(data){
	                    if(data){
                            jqcc.cometchat.addUsertoGroup(groupid,data.id);
	                    }
	                }
	            });
			} else {
	            jqcc.ajax({
	                type: "POST",
	                data: {groupid: groupid, users: userid, 'api-key': apikey, grouptype : groupType},
	                url: baseUrl+"api/addgroupusers",
	                dataType: 'html',
	                success: function(responsedata){
	                    ...
	                    if(data.success){
	                        ...
	                        addUser['message'] = data.success['message'];
	                        console.log(addUser);
	                    }
	                    else{
	                        addUser['userid'] = userid;
	                        addUser['groupid'] = groupid;
	                        addUser['message'] = data.failed['message'];
	                        console.log(addUser);
	                    }
	                }
	            });
		...
		arguments.callee.getGroupList = function(groupid){
			if(typeof groupid == 'undefined'){
				groupid = 0;
			}
			var groupname = [];
			var groups = jqcc.cometchat.getChatroomVars('chatroomdetails');
			if(groupid == 0){
				for(group in groups){
					groupname[group] = groups[group].name;
				}
			} else{
				for(group in groups){
					if(groups[group].id == groupid){
						groupname[groups[group].name] = groups[group];
					}
				}
			}
			return groupname;
		},
		arguments.callee.broadcast = function(id,chatroommode,cccloud){
			if(typeof(id) == 'undefined' || id == 0 || id == ''){
				return;
			}

			if(typeof(chatroommode) == 'undefined'){
				chatroommode = 1;
			}

			if(typeof(jqcc.cometchat.broadcastWithUID) == 'function' && typeof(cccloud) != 'undefined'){
			   jqcc.cometchat.broadcastWithUID(id,chatroommode);
			   return;
		    }
			...
			if(!isExist && chatroommode == 1){
				return;
			}
			var caller = jqcc.cometchat.getUserID();
			var getActivePlugins = jqcc.cometchat.getSettings().plugins;

			if(!getActivePlugins.includes('broadcast')){
				var broadcastDisabled = jqcc.ccbroadcast.getLanguage('broadcast_disabled');
				alert(broadcastDisabled);

				return;
			}
			jqcc['ccbroadcast'].init({'to':id, 'caller':caller, 'chatroommode': chatroommode});
		}
	};
...
function cometready(){
	jqcc(document).ready(function() {
		...
		<?php if(!defined('USE_COMET') || USE_COMET == 0 || (USE_COMET==1 && getCometServiceVersion()==2)) { ?>
