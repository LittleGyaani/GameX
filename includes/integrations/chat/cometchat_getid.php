...
while($chat = sql_fetch_assoc($query)){
	...
	if(strpos($fetchid,',')>0){
		$response[] =  array('id' => $chat['userid'], 'n' => $chat['username'], 'l' => $link, 'd' => $device,'a' => $avatar, 's' => $chat['status'], 'm' => $chat['message'], 'ch' => $chat['ch'], 'ls' => $chat['lastseen'], 'lstn' => $chat['lastseensetting'],'rdrs'=>$chat['readreceiptsetting']);
	}else{
		$response =  array('id' => $chat['userid'], 'n' => $chat['username'], 'l' => $link, 'd' => $device,'a' => $avatar, 's' => $chat['status'], 'm' => $chat['message'], 'ch' => $chat['ch'], 'ls' => $chat['lastseen'], 'lstn' => $chat['lastseensetting'],'rdrs'=>$chat['readreceiptsetting']);
	}
